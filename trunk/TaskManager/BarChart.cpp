...
void CBarChart::Print(BOOL bCompatible)
{
	...
	m_printer.Print(&dc, &m_dcMem, m_pBmpBars, 
		(CString)AfxGetAppName() + (CString)_T(" - ") + (CString)m_szTitle, 
...
void CBarChart::SetItemData(int nIndex,double dVal)
{
	m_bars.SetItemData(nIndex,dVal);
}
...
BOOL CBarChart::CreateFromStatic( UINT nStaticID, CWnd* pParent )
{
	CStatic  wndStatic;		
	...
	if ( !Create(rectStatic,pParent,nStaticID,NULL) )
	{		
...
BOOL CBarChart::SaveToFile(CString szPath)
{
	...
	if (szPath==_T(""))
	{
		...
		{
			m_szLastErr = _T("Canceled by user");
			return FALSE;
		}
	}
...
void CBarChart::CChartBar::CreateLabelFont()
{
	...
	lstrcpy(m_lf.lfFaceName,_T("Tahoma"));
	m_lf.lfHeight=-10;
	m_lf.lfWidth=0;
	m_lf.lfEscapement = 0;
...
void CBarChart::CChartBar::SetItemData(int nIndex,double dVal)
{
	m_chartData.SetItemData(nIndex,dVal);
}
...
void CBarChart::CChartData::SetItemData(int nIndex,double dVal)
{
	if (nIndex<0 || nIndex>=m_arr.GetSize())
	{
		return ;
	...
	((data*)m_arr.GetAt(nIndex))->m_udValue=dVal;
}
 ///////////////////////////////////////////////////////////////////////
 // CChartPrinter Class										 
 //																 
