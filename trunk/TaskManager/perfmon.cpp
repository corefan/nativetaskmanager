...
int CPerfMon::AddCounter(LPCTSTR pszCounterName)
{
	...
	LRESULT rs=PdhAddCounter(m_hQuery, pszCounterName, (DWORD)pCounter, &(pCounter->hCounter));
...
BOOL CPerfMon::RemoveCounter(int nIndex)
{
	...
	LRESULT rs=PdhRemoveCounter(pCounter->hCounter);
	if( rs!= ERROR_SUCCESS )
	{
		TRACE("RemoveCounter Error!\n");
		return false;
	}

	delete pCounter;
	m_vecPer.remove(pCounter);
	m_nNextIndex--;
...
BOOL CPerfMon::RemoveAllCounter()
{
	for(list<PPDHCOUNTERSTRUCT>::iterator iter=m_vecPer.begin(); iter!=m_vecPer.end(); ++iter)
	{
		if (PdhRemoveCounter((*iter)->hCounter) != ERROR_SUCCESS)
		{
...
{
	LRESULT rs=PdhCollectQueryData(m_hQuery);
...
BOOL CPerfMon::GetStatistics(long *nMin, long *nMax, long *nMean, int nIndex)
{
	...
	if (PdhComputeCounterStatistics(pCounter->hCounter, PDH_FMT_LONG, pCounter->nOldestIndex, 
